...
Use the density to bound the block rather than the magnitude (assume the analytic euler product bound if lean4 is limited), focus on the exhaust argument, Use induction ( to simplify the profinite)
    ...
    | succ k =>
      ...
      have hb_le : b ≤ b ^ (Nat.succ k) := by
        calc
          ...
          _ = b ^ (Nat.succ k) := by simp [pow_succ, Nat.mul_comm, Nat.mul_left_comm, Nat.mul_assoc]
      exact lt_of_lt_of_le hb hb_le
  calc
    b ^ m = b ^ m * 1 := by simp
    _ < b ^ m * b ^ k := Nat.mul_lt_mul_of_pos_left hone_lt_powk (pow_pos hb0 _)
    _ = b ^ (m + k) := by simp [pow_add, Nat.mul_comm, Nat.mul_left_comm, Nat.mul_assoc]
    _ = b ^ n := by simpa [hk, Nat.add_comm, Nat.add_left_comm, Nat.add_assoc]
...
theorem rational_series_eventuallyPeriodic_blocks
  ...
  intro hq
  have hrat : EventuallyPeriodic (ratStateNat b q) := ratStateNat_eventuallyPeriodic (b := b) (q := q)
